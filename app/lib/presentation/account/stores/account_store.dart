...
import 'package:encointer_wallet/presentation/account/types/tx_status.dart';
...
abstract class _AccountStore with Store {
  _AccountStore() : rootStore = sl.get<AppStore>();

  final AppStore rootStore;

  /// TODO(Azamat): Fix AccounData.name non-nullable
  /// Map<String, dynamic> acc cannot be null, it should be Map<String, dynamic> acc
  Map<String, dynamic> _formatMetaData(Map<String, dynamic> acc, {String? name}) {
    Log.d('_formatMetaData: acc = $acc, name = $name', _tag);
    if (name.isNotNullOrEmpty) {
      acc['name'] = name;
    } else {
      final meta = acc['meta'] as Map<String, dynamic>;
      if (meta['name'] == null) {
        acc['name'] = '';
      } else {
        acc['name'] = meta['name'] ?? '';
    ...
    Log.d('_formatMetaData: return acc = $acc', _tag);
    return acc;
  }
  ...
  String getNetworkAddress(String? pubKey) {
    Log.d('getNetworkAddress: pubkey = $pubKey', _tag);
  ...
  Future<void> setCurrentAccount(String? pubKey) async {
    Log.d('setCurrentAccount: pubkey = $pubKey', _tag);
  ...
  Future<void> updateAccountName(AccountData account, String newName) async {
    Log.d('updateAccountName: account = $account, newName = $newName', _tag);
  ...
  Future<void> updateAccount(Map<String, dynamic> acc) async {
    Log.d('updateAccount: acc = $acc', _tag);
  ...
  Future<void> addAccount(Map<String, dynamic> acc, String password, {String? name}) async {
    Log.d('addAccount: acc = $acc, password = $password, name = $name', _tag);
