...
describe('encointer', () => {
  ...
  const CANTILLON = 'wss://cantillon.encointer.org';
  let keyring;
  let registry;
  beforeAll(async () => {
    jest.setTimeout(90000);
    keyring = new Keyring({ type: 'sr25519' });
    await connect(CANTILLON);
    registry = window.api.registry;
    await setWorkerEndpoint(WORKER_URL);
  ...
  describe('init api', () => {
    it('should get cantillon genesis hash', async () => {
      ...
      const cantillonGenesisHash = '0x2b673afeff4a17e65fb3248fe9ac2a74998508a5c434f79a722af4fa8ab7470f';
      expect(u8aToHex(api.genesisHash)).toBe(cantillonGenesisHash);
      // assert(true);
    });
  });
  ...
  describe('substratee call_worker', () => {
    it('registerParticipant works', async () => {
      ...
      const cid = '3rk5pLBVZsWesSD4buAkZ4meguYKTpK8bKAA9MjtxPDe';
      const cidTyped = createType(registry, 'CurrencyIdentifier', bs58.decode(cid));
      ...
      const txRes = await _sendTrustedTx(alice, cid, cypherText);
      console.log(txRes);

      const participantIndex = await useWorker(window.workerEndpoint).getRegistration(alice, cid);
