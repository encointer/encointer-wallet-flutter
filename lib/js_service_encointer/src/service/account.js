...
import { TrustedCalls } from '../config/trustedCalls';
...
export async function txFeeEstimate (txInfo, paramList) {
  ...
  let dispatchInfo;
  if (window.settings.endpointIsCantillon() && TrustedCalls[txInfo.module][txInfo.call] !== null) {
    // Currently, all trusted calls encrypt to 384 bytes
...
export function _sendTrustedTx (keyPair, txInfo, paramList) {
  console.log('cid in params: ' + paramList[0]);
  // Todo: remove hardcoded mrenclave
  const mrenclave = api.createType('Hash', bs58.decode('DxF5PNys87yxxUSqD2CR37vi7UWLfrQ8fR5j6QUbWNXH'));
  const cid = api.createType('CurrencyIdentifier', paramList[0]);
  ...
  const call = createTrustedCall(
    ...
    mrenclave,
    nonce,
    TrustedCalls[txInfo.module][txInfo.call],
    [keyPair.pubKey, ...paramList]
  );
  ...
  const c = createType(api.registry, 'Vec<u8>', compactAddLength(cypherArray));
  console.log('Encrypted trusted call. Length: ' + cypherArray.length);

  const txParams = [api.createType('Request', {
    shard: api.createType('ShardIdentifier', paramList[0]),
    cyphertext: c
  })];
