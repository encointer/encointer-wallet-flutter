...
import { hexToU8a, u8aToHex, hexToString, assert } from '@polkadot/util';
...
function sendTrustedTx (pubKey, cid, trustedCallString, trustedCallParams, password) {
  ...
  const call = createTrustedCall(keyPair, cid, trustedCallString, trustedCallParams);
  const callEncrypted = window.workerShieldingKey.encrypt(call.toU8a(), 'hex');
  return _sendTrustedTx(keyPair, cid, callEncrypted);
}

export function _sendTrustedTx (keyPair, cid, trustedCallEncrypted) {
  const txParams = [api.createType('Request', {
    shard: api.createType('ShardIdentifier', cid),
    // cyphertext: api.createType('Vec<u8>', trustedCallEncrypted)
    cyphertext: trustedCallEncrypted
  })];
  ...
  return _sendTx(keyPair, txInfo, txParams);
}

const RQ_TRUSTED_CALL_NAMES = Object.keys(CustomTypes.TrustedCall._enum);
const RQ_TRUSTED_CALL_TYPES = Object.values(CustomTypes.TrustedCall._enum);

export function createTrustedCall (account, cid, trustedCall, params) {
  const callIndex = RQ_TRUSTED_CALL_NAMES.indexOf(trustedCall);
  assert(callIndex !== -1, `Unknown request: ${trustedCall}`);

  const call = createType(api.registry, 'TrustedCall', {
    [trustedCall]: createType(api.registry, RQ_TRUSTED_CALL_TYPES[callIndex], params)
  });

  const signature = account.sign(call.toU8a());
