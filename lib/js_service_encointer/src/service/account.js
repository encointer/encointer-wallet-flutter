...
import { encodeDoubleToI64F64, toI64F64 } from '../utils/fixpointUtil';
...
export async function txFeeEstimate (txInfo, paramList) {
  if (txInfo.module === 'encointerBalances' && txInfo.call === 'transfer') {
    paramList[2] = encodeDoubleToI64F64(parseFloat(paramList[2]));
  }
...
export function sendTx (txInfo, paramList) {
  return new Promise((resolve, reject) => {
    ...
    if (txInfo.module === 'encointerBalances' && txInfo.call === 'transfer') {
      paramList[2] = api.createType('i128', encodeDoubleToI64F64(parseFloat(paramList[2])));
    }
