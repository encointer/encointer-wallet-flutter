...
let RQ_TRUSTED_GETTER_NAMES = Object.keys(CustomTypes.TrustedGetter._enum);
...
const parseBalance = data => {
  ...
  window.send('js-parseBalanceEntry', `BalanceEntry:  ${balanceEntry}`);
  return {
...
const parseBalanceType = data => {
  return parseI64F64(u8aToBn(unpack(data)));
};
...
const parseMeetupIndex = data => {
  window.send('js-parseMeetupIndex', `got meetup index: ${data}`);
  return createType(api.registry, 'MeetupIndexType', unpack(data));
};

const parseRegistry = data => {
  return createType(api.registry, 'Vec<AccountId>', unpack(data));
};

const parseWorkerPubKey = data => {
  const keyJson = JSON.parse(data);
  window.send('log', 'WorkerShieldingKeyJson: { n: ' + keyJson.n + 'e: ' + keyJson.e + '}');
  return rsa.nodeRsaParseRawJsonKey(keyJson);
};
...
function requestParams (address, shard) {
  return createType(api.registry, '(AccountId, CommunityIdentifier)', [address, shard]);
}
...
function clientRequestTrustedGetter (account, cid, request) {
  ...
  const cidBin = u8aToHex(bs58.decode(cid));
  window.send('js-clientRequestTrustedGetter', `request: ${request}`);
  window.send('js-clientRequestTrustedGetter', `address: ${address}`);
  window.send('js-clientRequestTrustedGetter', `cidBin: ${cidBin}`);
...
export class Worker {
  ...
  getRegistration (account, cid) {
    return this.sendRequest(clientRequestTrustedGetter(account, cid, 'participant_index'), parseParticipantIndex);
  }

  getMeetupIndex (account, cid) {
    return this.sendRequest(clientRequestTrustedGetter(account, cid, 'meetup_index'), parseMeetupIndex);
  }
  ...
  getMeetupRegistry (account, cid) {
    return this.sendRequest(clientRequestTrustedGetter(account, cid, 'meetup_registry'), parseRegistry);
  }
}

/**
 * Updates the TrustedGetter names found in the config/types.js. Currently, this is only used in the tests if a local
 * network has a different TrustedGetter type than the default.
 */
export function updateTrustedGetterTypes (trustedGetterEnum) {
  RQ_TRUSTED_GETTER_NAMES = Object.keys(trustedGetterEnum._enum);
}
...
async function getParticipantCount (cid) {
  return useWorker(window.workerEndpoint).getParticipantCount(cid);
}

// wrappers to fetch account from keyring
async function getBalance (pubKey, cid, password) {
...
// wrappers to fetch account from keyring
async function getParticipantIndex (pubKey, cid, password) {
  const keyPair = keyring.getPair(hexToU8a(pubKey));
  try {
    keyPair.decodePkcs8(password);
  } catch (err) {
...
async function getMeetupIndex (pubKey, cid, password) {
  const keyPair = keyring.getPair(hexToU8a(pubKey));
  try {
    keyPair.decodePkcs8(password);
  } catch (err) {
  ...
  return useWorker(window.workerEndpoint).getMeetupIndex(keyPair, bs58.encode(hexToU8a(cid)));
}

async function getAttestations (pubKey, cid, password) {
  const keyPair = keyring.getPair(hexToU8a(pubKey));
  try {
    keyPair.decodePkcs8(password);
  } catch (err) {
  ...
  return useWorker(window.workerEndpoint).getAttestations(keyPair, bs58.encode(hexToU8a(cid)));
}

async function getMeetupRegistry (pubKey, cid, password) {
  const keyPair = keyring.getPair(hexToU8a(pubKey));
  try {
    keyPair.decodePkcs8(password);
  } catch (err) {
  ...
  return useWorker(window.workerEndpoint).getMeetupRegistry(keyPair, bs58.encode(hexToU8a(cid)));
}
