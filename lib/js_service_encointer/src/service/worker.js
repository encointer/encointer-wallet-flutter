...
const parseWorkerPubKey = data => {
  ...
  keyJson.n = Buffer.from(u8aToHex(keyJson.n).substring(2), 'hex');
  keyJson.e = Buffer.from(u8aToHex(keyJson.e).substring(2), 'hex');
  console.log(keyJson);
  const key = new NodeRSA();
  key.setOptions({ encryptionScheme: { scheme: 'pkcs1_oaep', hash: 'sha256', label: null } });
  key.importKey(keyJson, 'components-public');
  console.log(key);
  return key;
};
