...
export async function remainingNewbieTicketsReputable (cid, ceremonyIndex, pubkey) {
  const cidT = api.createType('CommunityIdentifier', cid);
  window.send('js-remainingNewbieTickets-reputable', `cid: ${communityIdentifierToString(cidT)}, cIndex: ${ceremonyIndex} and address: ${pubkey}`);
...
export async function remainingNewbieTicketsBootstrapper (cid, address) {
  const cidT = api.createType('CommunityIdentifier', cid);
  window.send('js-remainingNewbieTickets-bootstrapper', `cid: ${communityIdentifierToString(cidT)}, and address: ${address}`);
