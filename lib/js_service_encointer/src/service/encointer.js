...
export async function _signClaimOfAttendance (claimant, claimJson) {
  ...
  const location = {
      lat: encodeFloatToI64F64(parseFloat(claimJson.location.lat)).reverse(),
      lon: encodeFloatToI64F64(parseFloat(claimJson.location.lon)).reverse(),
  }

  let claim = {
    ...claimJson,
     location: api.createType('Location', location),
  }

  send("js-signClaimOfAttendance", `claim: ${JSON.stringify(claim)}`)

  const payload = api.createType('ClaimOfAttendanceSigningPayload', claim);
