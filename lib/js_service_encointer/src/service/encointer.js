...
export async function fetchParticipantCount (cid, cIndex) {
  send('log', `Fetching participant count for Cid: ${cid} and cIndex: ${cIndex}`);
  return await api.query.encointerCeremonies.participantCount([cid, cIndex]);
}

export async function fetchMeetupRegistry (cid, cIndex, mIndex) {
  send('log', `Fetching meetupRegistry for Cid: ${cid}, cIndex: ${cIndex} and mIndex: ${mIndex}`);
...
export async function attestClaimOfAttendance (claimHex, pubKey, password) {
  ...
  return new Promise((resolve, reject) => {
    ...
    send('log', `got keypair encoded: ${keyPair}`);
    try {
      keyPair.decodePkcs8(password);
    } catch (err) {
      resolve({ error: 'password check failed' });
    }
    send('log', `attestingClaimOfATtendance with claim: ${claimHex} and account: ${pubKey}`);

    const publicKey = createType(registry, 'AccountId', pubKey).toU8a();
    send('log', `Got public key ${publicKey}`);
    const sign = pubKey.sign(claimHex);
    send('log', `Got sign ${sign}`);
