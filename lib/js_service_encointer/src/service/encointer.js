import { assert, bnToHex, bnToU8a, hexToU8a, u8aToHex } from '@polkadot/util';
...
export async function _signClaimOfAttendance (claimant, claimJson) {
  ...
  const payload = api.createType('ClaimOfAttendanceSigningPayload', {
    ...claim,
    location: {
      // scale-coded is LE, hence we need to sign the LE encoded payload, which is not the default for `i128`, otherwise
