...
export async function getParticipantIndex (cid, cIndex, address) {
  send('js-getParticipantIndex', `Getting participant index for Cid: ${communityIdentifierToString(cid)}, cIndex: ${cIndex} and address: ${address}`);
...
export async function getParticipantCount (cid, cIndex) {
  send('js-getParticipantCount-', `Getting participant count for Cid: ${communityIdentifierToString(cid)} and cIndex: ${cIndex}`);
...
export async function getParticipantReputation (cid, cIndex, address) {
  send('js-getParticipantReputation', `Getting participant reputation for Cid: ${communityIdentifierToString(cid)}, cIndex: ${cIndex} and address: ${address}`);
...
export async function getCommunityIdentifiers () {
  ...
  // send('js-getCommunityIdentifiers', `Got Cids Raw: ${communityIdentifierToString(cid)}`);
...
export async function getMeetupRegistry (cid, cIndex, mIndex) {
  send('js-getMeetupRegistry', `Getting meetupRegistry for Cid: ${communityIdentifierToString(cid)}, cIndex: ${cIndex} and mIndex: ${mIndex}`);
...
export async function getProofOfAttendance (attendeePubKey, cid, cIndex, password) {
  window.send('js-getProofOfAttendance', `getting PoA for  Cid: ${communityIdentifierToString(cid)}, cIndex: ${cIndex} and address: ${attendeePubKey}`);
