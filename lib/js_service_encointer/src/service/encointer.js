...
export async function subscribeShopRegistry (msgChannel, cid, shops) {
  return await api.query.encointerBazaar.shopRegistry((cids) => {
    send(msgChannel, cids);
  }).then((unsub) => unsubscribe(unsub, msgChannel));
}
...
export async function getShopRegistry (cid) {
  const shops = await api.query.encointerBazaar.shopRegistry(cid);
  return shops;
}
