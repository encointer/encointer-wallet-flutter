...
export async function fetchParticipantIndex(cid, address) {
  ...
  const p_index = api.query.encointerCeremonies.participantIndex([cid, c_index], address);
 ...
 */
export async function fetchNextMeetupLocation(cid, account) {
  const c_index = await api.query.encointerScheduler.currentCeremonyIndex();
  send("log", `Current Ceremony Index: ${c_index}`);
  const m_index = await api.query.encointerCeremonies.meetupIndex([cid, c_index], account)
