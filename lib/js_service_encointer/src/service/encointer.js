...
export async function getParticipantReputation (cid, cIndex, address) {
  send('js-getParticipantIndex', `Getting participant reputation for Cid: ${cid}, cIndex: ${cIndex} and address: ${address}`);
  const reputation = await api.query.encointerCeremonies.participantReputation([cid, cIndex], address);
  send('js-getParticipantIndex', `Participant index: ${reputation}`);
  return reputation;
}
