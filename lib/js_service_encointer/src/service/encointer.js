...
export async function reapVoucher (voucherUri, recipientAddress, cid) {
  ...
  const balanceEntry = await getBalance(cid, voucherPair.address);

  // keep enough balance for the recipientAddress to pay the fees
  const transferAmount = balanceEntry.principal - 0.02;

  console.log(`[encointer/reapVoucher] ${transferAmount}`);
