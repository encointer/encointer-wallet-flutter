...
import { encodeFloatToI64F64 } from '@encointer/util';
...
export async function getParticipantIndex (cid, cIndex, address) {
  send('js-getParticipantIndex', `Getting participant index for Cid: ${JSON.stringify(cid)}, cIndex: ${cIndex} and address: ${address}`);
...
export async function getParticipantCount (cid, cIndex) {
  send('js-getParticipantCount-', `Getting participant count for Cid: ${JSON.stringify(cid)} and cIndex: ${cIndex}`);
...
export async function getMeetupRegistry (cid, cIndex, mIndex) {
  send('js-getMeetupRegistry', `Getting meetupRegistry for Cid: ${JSON.stringify(cid)}, cIndex: ${cIndex} and mIndex: ${mIndex}`);
...
export async function getNextMeetupTime (location, phase, duration) {
  ...
  send('js-getNextMeetupTime:', `Location: ${JSON.stringify(location)}`)
...
export async function _signClaimOfAttendance (claimant, claimJson) {
  ...
  // location containing degree as string, e.g. 35.23432



  const location = api.createType('Location',  {
      lat: u8aToHex(api.createType('DegreeFixed',  encodeFloatToI64F64(parseFloat(claimJson.location.lat))).toU8a()),
      lon: u8aToHex(api.createType('DegreeFixed',  encodeFloatToI64F64(parseFloat(claimJson.location.lon))).toU8a()),
  });

  send("js-signClaimOfAttendance", `location: ${location.toU8a()}`)
  ...
  send("js-signClaimOfAttendance", `claim: ${JSON.stringify(claim)}`)

  const payload = api.createType('ClaimOfAttendanceSigningPayload', { ...claim, location: {
