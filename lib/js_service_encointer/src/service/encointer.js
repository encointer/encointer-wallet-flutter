...
export async function getCommunityMetadata (cid) {
  const pallet = pallets.encointerCommunities;
  send('js-getCommunityMetadata', `Getting community metadata for cid: ${cid}`);
  const meta = await api.query[pallet.name][pallet.communityMetadata](cid);
  send('js-getCommunityMetadata', `got metadata metadata: ${meta}`);
  return meta;
}
