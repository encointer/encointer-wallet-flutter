...
export async function remainingNewbieTickets (pubkey, ceremonyIndex, cid) {
  // Wrapping it in a promise all speeds up the process as we can await multiple futures at the same time.
  console.log(`remainingNewbieTickets called ${cid}`);
  const [burnedTickets, ticketsPerReputation] = await Promise.all([
    ...
    api.query.encointerCeremonies.endorsementTicketsPerReputatble(),
  ]).catch((e) => console.log(`remainingNewbieTickets ${e}`));
