...
import { parseI32F32, parseI64F64, toI32F32 } from '@encointer/util';
...
export async function subscribeCommunityIdentifiers (msgChannel) {
  return await api.query.encointerCurrencies.communityIdentifiers((cids) => {
...
export async function getCommunityIdentifiers () {
  const cids = await api.query.encointerCurrencies.communityIdentifiers();
