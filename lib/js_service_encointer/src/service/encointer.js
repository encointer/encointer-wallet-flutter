...
import { parseEncointerBalance, toDegree } from '@encointer/types';
...
export async function _signClaimOfAttendance (claimant, claimJson) {
  ...
  let claim = {
    ...claimJson,
    location: {
      lat: toDegree(parseFloat(claimJson.location.lat)),
      lon: toDegree(parseFloat(claimJson.location.lon)),
    }
  }
