import { assert, bnToBn, hexToU8a, u8aToHex, u8aToU8a } from '@polkadot/util';
...
export async function fetchParticipantIndex (cid, address) {
  send('log', `Fetching participant index for Cid ${cid} and address ${address}`);
  const c_index = await api.query.encointerScheduler.currentCeremonyIndex();
  send('log', `Current Ceremony Index: ${c_index}`);
  const p_index = await api.query.encointerCeremonies.participantIndex([cid, c_index], address);
  send('log', `Participant index: ${p_index}`);
  return p_index;
}

export async function fetchParticipantCount (cid) {
  send('log', `Fetching participant cound for Cid ${cid}`);
  const c_index = await api.query.encointerScheduler.currentCeremonyIndex();
  send('log', `Current Ceremony Index: ${c_index}`);
  const p_count = await api.query.encointerCeremonies.participantCount([cid, c_index]);
  return p_count;
}

export async function fetchNextMeetupTime (cid, address) {
  send('log', `Fetching next meetup time for Cid ${cid} and address ${address}`);
  const phase = await api.query.encointerScheduler.currentPhase();
  ...
  const next_phase = await api.query.encointerScheduler.nextPhaseTimestamp();
  send('log', `Next Phase Timestamp: ${next_phase}`);
  const location = await fetchNextMeetupLocation(cid, address);
  send('log', `Location: ${location}`);
  const moments_per_day = await api.query.encointerCeremonies.momentsPerDay();
  // const per_degree = moments_per_day / 360;
  send('log', `moments per day: ${moments_per_day}`);
 ...
 */
export async function fetchNextMeetupLocation (cid, address) {
  const c_index = await api.query.encointerScheduler.currentCeremonyIndex();
  send('log', `Current Ceremony Index: ${c_index}`);
  const m_index = await api.query.encointerCeremonies.meetupIndex([cid, c_index], address);
  // assert(m_index !== 0, "Meetup index is null, hence no meetup was found")
  send('log', `Meetup Index: ${m_index}`);
  const locations = await api.query.encointerCurrencies.locations(cid);
  send('log', `Locations: ${locations}`);
  assert(m_index <= locations.length);
  const _loc = locations[m_index - 1];
 ...
 */
export function getClaimOfAttendance (accountid, cindex, cid, mindex, location, timestamp, participants) {
