...
import account, { _sendTrustedTx, createTrustedCall } from '../src/service/account';
import { bufferToU8a, hexToU8a, u8aToBuffer, u8aToHex } from '@polkadot/util';
...
import { localDockerNetwork } from './utils';
    ...
    describe('substratee call_worker', () => {
      it('registerParticipant works', async () => {
        ...
        const call = createTrustedCall(
          ...
          (alice.publicKey, cidTyped, proof)
        );

        console.log(call);

        const cypherText = window.workerShieldingKey.encrypt(u8aToBuffer(call.toU8a()), 'hex');
        console.log('cyphertext: ' + cypherText);
        console.log('cyphertext: ' + hexToU8a('0x' + cypherText));
        console.log('cyphertext: ' + hexToU8a('0x' + cypherText).length);

        const c = createType(registry, 'Vec<u8>', '0x' + cypherText);
