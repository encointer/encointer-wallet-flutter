...
describe.skip('cantillon', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      window.send('log', `${network.chain} wss connected success`);

      // for cases where the code calls the api global directly
      global.api = window.api;
    } catch (err) {
      window.send('log', `connect ${network.chain} failed`);
