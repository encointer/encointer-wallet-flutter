...
abstract class PrepareMockStorage {
  ...
  static void homePage(AppStore store) {
    ...
    store.encointer
        .setNextPhaseTimestamp(now + Duration(hours: 8).inMilliseconds);
    store.encointer.community!
        .setCommunityMetadata(CommunityMetadata.fromJson(communityMetadata));
    store.encointer.community!.setDemurrage(demurrage);
    store.encointer.account!
        .addBalanceEntry(cid, BalanceEntry.fromJson(testBalanceEntry));
    store.encointer.community!
        .setMeetupTime(now + Duration(hours: 8).inMilliseconds);
  }
  ...
  static void readyForMeetup(AppStore store) {
    ...
    store.encointer.community!
        .setMeetupLocations([testLocation1, testLocation2, testLocation3]);
