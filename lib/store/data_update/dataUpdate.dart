...
abstract class _DataUpdateStore with Store {
  ...
  @action
  setInvalidated() => invalidated = true;
  ...
  void setupUpdateReaction(Future<void> Function() updateFn) {
    ...
    _disposer = reaction((_) => now, (_) {
      if (needsRefresh) {
        Log.d("Reaction triggered...", 'account.dart');
        executeUpdate();
      } else {
  ...
  Future<void> executeUpdate() async {
    if (_updateFn == null) {
      Log.d("No `updateFn` set, returning...", 'account.dart');
      return;
    }

    if (_updateFuture != null) {
      Log.d("already updating, awaiting the previously set future.", 'account.dart');
      await _updateFuture!;
      return;
    }
    ...
    _updateFuture = _updateFn!().timeout(Duration(seconds: 15)).then((value) {
      ...
      Log.d("Error while executing `updateFn`: ${e.toString()}", 'account.dart');
    }).whenComplete(() {
      _updateFuture = null;
      Log.d("update reaction finished", 'account.dart');
    });
