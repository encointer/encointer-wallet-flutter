...
class _CreatePinPageState extends State<CreatePinPage> {
  ...
  Widget build(BuildContext context) {
    CreatePinPageParams params =
        ModalRoute.of(context)!.settings.arguments as CreatePinPageParams;
            ...
            ? CreatePinForm(
                onSubmit: () async {
                  ...
                  if (store.encointer.communityIdentifiers.length == 1) {
                    store.encointer
                        .setChosenCid(store.encointer.communityIdentifiers[0]);
                  } else {
                    await Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => CommunityChooserOnMap(store)),
                    );
                  }
