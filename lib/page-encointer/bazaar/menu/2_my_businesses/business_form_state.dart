...
abstract class _BusinessFormState with Store {
  ...
  void validateDescription(value) {
    return validateIsNotBlank(
        value, (errorText) => errors.description = errorText);
  }
  ...
  void validateStreetAddendum(value) {
    return validateIsNotBlank(
        value, (errorText) => errors.streetAddendum = errorText);
  }
  ...
  validateIsNotBlank(String? value, Function(String?) errorTarget) {
    String? errorText =
        value == null || value.trim().isEmpty ? 'Cannot be blank' : null;
...
class BusinessFormErrorState = _BusinessFormErrorState
    with _$BusinessFormErrorState;
