...
class OpeningHoursForDayState = _OpeningHoursForDayState
    with _$OpeningHoursForDayState;
  ...
  addParsedIntervalIfValid(String startEnd) {
    try {
      OpeningIntervalState openingIntervalState =
          _OpeningIntervalState.parseOpeningIntervalState(startEnd)
              as OpeningIntervalState;
...
abstract class _OpeningIntervalState with Store {
  ...
  static _OpeningIntervalState parseOpeningIntervalState(String startEndTime) {
    return OpeningIntervalState(_parseTimeInterval(startEndTime, 0),
        _parseTimeInterval(startEndTime, 1));
  }
  ...
  static int _parseTime(String time) {
    ...
    var timeClean = indexOfMeridiem > 0
        ? timeLowerCase.substring(0, indexOfMeridiem)
        : timeLowerCase;
    ...
    var minutes =
        hoursMinutes.length > 1 ? int.parse(hoursMinutes[1].trim()) : 0;
