...
class BMap extends StatelessWidget {
  ...
  BMap(List<BazaarItemData> data)
      ...
      : businessData = data
            .where((item) => item is BazaarBusinessData)
            .map((item) => item as BazaarBusinessData)
            .toList() {
    // construct a map using "collection for"
    bazaarBusinessDataFor.addAll(
        {for (var business in businessData) business.coordinates: business});
  }
          ...
          options: PopupMarkerLayerOptions(
            ...
            popupBuilder: (BuildContext context, Marker marker) =>
                BusinessDetailsPopup(
                    marker, bazaarBusinessDataFor[marker.point]),
          ),
        ),
      ],
    );
...
class BusinessDetailsPopup extends StatelessWidget {
  ...
  BusinessDetailsPopup(this.marker, this.dataForThisMarker, {Key? key})
      : super(key: key);
}
